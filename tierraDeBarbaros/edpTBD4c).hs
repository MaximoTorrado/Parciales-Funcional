...
--Misma pero usando composicion
tieneFuerzaMayorA' :: Int -> Evento
tieneFuerzaMayorA' unaFuerza unBarbaro =  ((> unaFuerza) . fuerzaDeBarbaro ) unBarbaro
...
-- c) Pregunta. ¿Se podria aplicar "sinRepetidos" sobre la lista de objetos? ¿Y sobre el nombre de un barbaro? ¿Porque?
-- Respuesta:

{- Si miramos el tipado de "sinRepetidos" vemos que los parametros que recibe deben ser comparables, si queremos aplicar esto en una lista de objetos de un barbaro no podriamos ya
que los elementos de esta lista son funciones, y las funciones no son comparables, si se podria hacer en el nombre de un barbaro, ya que este es de tipo String, que es un Alias de 
[Char] entonces puede comparar elemento a elemento los caracteres de dicho String, entonces nos quedaria una cadena sin repetir caracteres -}
